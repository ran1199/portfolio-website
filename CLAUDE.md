...
- **Current phase:** Phase 4 (Case study pages) — Galactic Guardians visual-first (published). Buddy's Adventure Island visual-first framework (published). Ran's plan: build the visual-first framework for every case study first (CTE and WonderWeave next, from Ran's update files), then Ran sends images and text per project.
