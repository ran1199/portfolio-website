...
- **Home gallery (Phase 3, revised after Ran's feedback with zegzulka.com screenshots):** a filled grid with no white space, small even gaps (`--gallery-gap`) and rounded corners (`--radius-media`). Wide screens (≥64rem): 3 columns; each case study spans 2 columns × 2 rows, alternating left/right, with its 2 visual pieces stacked beside it; rows sized from the container width (container query units). Below 64rem: 2 columns; case study full width with details under the image, then 2 pieces side by side. Home fits 8 pieces exactly (`home: true` in `play.yaml`); extras fill rows of 3 below. Case study details sit on a white panel in the image's bottom-left corner, fading in on hover (always visible on wide touch screens). Visual pieces show optional `title` + `tags` labels on hover (mouse/trackpad only) and are never links. The "View" cursor (`src/components/ViewCursor.astro`) appears over anything marked `data-cursor="view"`, only on `(hover: hover) and (pointer: fine)`. Scroll fade-in: add `class="reveal"`.
- **Case study pages (Phase 4):** `/work/<file-name>/`, from `src/pages/work/[slug].astro`. All content is in the file's front matter: hero fields, then sections `why`, `research`, `insight`, `concept`, `prototype` (`built` / `designed`), `testing` (`before` / `after`), `reflection`, optional `credits` and `notice`. Sections are lists of "parts" built from blocks (heading, label, text, points, pairs, steps, visuals, video, source), rendered by `src/components/case/Part.astro`. `content/case-studies/_TEMPLATE.md` documents every option (files starting with `_` aren't published). `[PLACEHOLDER…]` and `[SOURCE NEEDED]` are highlighted automatically. Section headings and labels live in `site.yaml` under `caseStudy`.
- **Visual-first case studies (Ran's update, Galactic Guardians):** a case study file with `sections:` uses the visual-first layout instead of the classic fixed sections: each section has a `heading` and `blocks` (image [+ insight/goal/text beside it], `size: small`, `cards` from→to, `gallery` 1 large + 2, `steps` photo row, `screens` app row, `before`/`after`, `video`, `note`, `text`+`source`), rendered by `src/components/case/Block.astro` inside stacked `Section`s numbered in order. "Why this project" and "Reflection" are optional there. Classic files (Buddy's, CTE, WonderWeave) still render exactly as before — verified by comparing built HTML. `_TEMPLATE.md` now documents the visual-first format. Rules: images lead; ≤2 short sentences per text; captions ≤12 words; never add animation beyond existing fades. GG images are labeled grey placeholders with the final file names, so Ran replaces them one-for-one. Buddy's Adventure Island is also visual-first (4 sections + `credits`, shown as small text after the last section). Extra block options added for it: `insight` may be a list of lines; `sideBySide: true` on a gallery puts all images in one equal row, with an optional shared `caption`; before/after panels use full width for landscape images. A gallery of lead + exactly one image shows both full width.
- **Publishing = pushing:** the working branch is the default branch, so every push publishes the live site. When Ran asks to approve before publishing, commit locally and push only after the OK.
- **Current phase:** Phase 4 (Case study pages) — Galactic Guardians visual-first (published). Buddy's Adventure Island visual-first framework built (awaiting Ran's review and OK to publish). Ran's plan: build the visual-first framework for every case study first (CTE and WonderWeave next, from Ran's update files), then Ran sends images and text per project.
